...
function projectPoint(point, time) {
  const cx = width < 720 ? width * 0.56 : width * 0.43;
  const cy = height * 0.42;
  const scale = Math.min(width, height) * (width < 720 ? 0.68 : 0.86);
  const spin = time * 0.00042;
...
function drawScanRings(time) {
  const cx = width < 720 ? width * 0.56 : width * 0.43;
  const cy = height * 0.42;
  const base = Math.min(width, height) * (width < 720 ? 0.22 : 0.27);
  ctx.save();
  ctx.translate(cx, cy);
...
function drawOreCore(time) {
  const cx = width < 720 ? width * 0.56 : width * 0.43;
  const cy = height * 0.42;
  const radius = Math.min(width, height) * (width < 720 ? 0.18 : 0.2);
...
function drawScanBeam(time) {
  const cx = width < 720 ? width * 0.56 : width * 0.43;
  const cy = height * 0.42;
  const radius = Math.min(width, height) * (width < 720 ? 0.45 : 0.54);
  const angle = time * 0.00135;
...
function drawOrbitMarkers(time) {
  const cx = width < 720 ? width * 0.56 : width * 0.43;
  const cy = height * 0.42;
  const radius = Math.min(width, height) * (width < 720 ? 0.34 : 0.43);
